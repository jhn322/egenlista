...
export async function middleware(request: NextRequest) {
  ...
  // For this project, primarily PROTECTED_PATHS.MINA_SIDOR_BASE
  const isProtectedRoute = nextUrl.pathname.startsWith(PROTECTED_PATHS.MINA_SIDOR_BASE);
  ...
  if (isProtectedRoute && !isAuthenticated) {
    ...
    console.log(`Middleware: Unauthenticated access to ${nextUrl.pathname}, redirecting to ${loginUrl.toString()}`);
    return NextResponse.redirect(loginUrl);
  }

  // If authenticated and trying to access login/register pages, redirect to default logged-in page.
  if (isAuthenticated && (nextUrl.pathname === AUTH_PATHS.LOGIN || nextUrl.pathname === AUTH_PATHS.REGISTER)) {
    console.log(`Middleware: Authenticated user accessing ${nextUrl.pathname}, redirecting to ${DEFAULT_LOGIN_REDIRECT_PATH}`);
    return NextResponse.redirect(new URL(DEFAULT_LOGIN_REDIRECT_PATH, nextUrl.origin));
  }
